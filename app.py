...
def init():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS users (
            id BIGINT PRIMARY KEY, balance NUMERIC(10,2) DEFAULT 0, ads_today INTEGER DEFAULT 0, 
            refs INTEGER DEFAULT 0, last_ad_date DATE, referrer_id BIGINT
        );
...
def webhook():
    ...
    if update.message:
        ...
        if update.message.text and "/start" in update.message.text:
            ...
            if d['action'] == 'ad':
                cur.execute("UPDATE users SET balance = balance + 5, last_ad_date = %s WHERE id = %s", (date.today(), uid))
            elif d['action'] == 'withdraw':
                cur.execute("INSERT INTO withdrawals (user_id, amount, method, number) VALUES (%s, %s, %s, %s)", (uid, d['amt'], d['method'], d['num']))
                cur.execute("UPDATE users SET balance = balance - %s WHERE id = %s", (d['amt'], uid))
            conn.commit()
            update.message.reply_text("অ্যাকশন সফল! ✅")
        cur.close()
        conn.close()
    return "ok"
